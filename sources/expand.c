...
static char *get_env_value(t_env *env_list, const char *var_name)
{
    while (env_list)
    {
        ...
        if (*start == '$' && (is_valid_var_char(*(start+1), true) || *(start+1) == '?'))
        {
            ...
            else
            {
                ...
                while (is_valid_var_char(*start, start == var_start))
                    start++;
            	var_name = strndup(var_start, start - var_start);
...
char *expand_input(char *input, int exit_status, t_env *env_list)
{
    ...
    char *expanded_tilde = expand_tilde(input);
    char *expanded_vars = expand_env_vars(expanded_tilde, exit_status, env_list);
    char *no_quotes = remove_quotes(expanded_vars);
